        ...
        <Card className="border-border p-5 lg:col-span-2">
          ...
          <ul className="mt-4 grid gap-3 sm:grid-cols-2">
            {predio.contactos.map((v) => {
              ...
              return (
                <li key={v.contactoId} className="rounded-md border border-border p-4">
                  <div className="flex items-center justify-between gap-2">
                    <p className="font-medium">{c?.nombre}</p>
                    <Badge variant="outline" className="bg-secondary text-secondary-foreground">
                      {v.rol === "propietario_principal" ? "Principal" : "Socio"}
                    </Badge>
                  </div>
                  <p className="mt-1 text-xs text-muted-foreground">Participación {v.participacion ?? "—"}%</p>
